...
async def create_session(
    ...
    Args:
        ...
        payment_id: Payment session ID (required for paid generation).
...
# ============================================================================
# User Ideas Operations (My Ideas)
# ============================================================================


async def get_all_user_ideas(
    session: AsyncSession,
    user_id: str,
    ...
    """Get all ideas generated by a user across all sessions.

    Args:
        session: Database session.
        user_id: User UUID.
        include_deleted: Whether to include soft-deleted ideas.
        bookmarked_only: Filter to only bookmarked ideas.
        limit: Maximum number of ideas to return.
        offset: Number of ideas to skip.
    ...
    conditions = [GeneratedIdea.session_id.in_(select(user_sessions))]

    if not include_deleted:
        conditions.append(GeneratedIdea.is_deleted == False)  # noqa: E712

    if bookmarked_only:
        conditions.append(GeneratedIdea.is_bookmarked == True)  # noqa: E712

    # Count total
    count_result = await session.execute(
        select(func.count(GeneratedIdea.id)).where(*conditions)
    )
    total = count_result.scalar_one()

    # Get ideas with session details
    result = await session.execute(
        select(GeneratedIdea)
        .options(selectinload(GeneratedIdea.session))
        .where(*conditions)
...
async def soft_delete_idea(
    session: AsyncSession,
    idea_id: str,
    user_id: str,
) -> Optional[GeneratedIdea]:
    """Soft delete an idea (set is_deleted = True).
...
async def restore_idea(
    ...
    """Restore a soft-deleted idea (set is_deleted = False).
